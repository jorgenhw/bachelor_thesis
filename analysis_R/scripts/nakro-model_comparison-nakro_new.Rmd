...
# nb-bert-large
classf_report_nb_bert_large <- list.files(path = "../data/nbailab", pattern = "*.csv", 
               full.names = T) %>% 
    map_df(~read_plus(.))
...
multi <- c("ScandiBERT", "xlm-roberta-large", "xlm-roberta-base", "electra-small-nordic", "mdeberta-v3-base", "twitter-xlm-roberta", "nb-bert-large")
...
func_TEMP <- function(topic){
  ...
  func_model <- ulam(
    alist(
        ...
        a_model[language_model] ~ dnorm(0, sigma_a),
        #a_group[language] ~ dnorm(0.4, 0.15),
        a_group[language_12] ~ dnorm(a_bar, sigma_bar),
        a_bar ~ dnorm(0, 0.2),
        sigma_bar ~ dexp(10),
        sigma_a ~ dexp(10),
        sigma ~ dexp(10)
...
analysis_df <-  model_topic_accuracy_group_df %>% 
  ...
  mutate(language_model = ifelse(model == 4, 1, 2)) %>% 
  #mutate(language_model = ifelse((model == 4)|(model == 5), 1, 2)) %>% # perhaps I need to have an indicator variablt that links the xlm models together and the nb-bert models together
  select(language_model, size_12, diff_topic, left_out)
...
func_TEMP <- function(topic){
  ...
  func_model <- ulam(
    alist(
        diff_topic ~ dnorm(mu, sigma),
        #mu <- a_model[language_model],
        mu <- a_model[language_model] + a_group[size_12],
        a_model[language_model] ~ dnorm(0, sigma_a),
        #a_group[language] ~ dnorm(0.4, 0.15),
        a_group[size_12] ~ dnorm(a_bar, sigma_bar),
        a_bar ~ dnorm(0, 0.2),
        sigma_bar ~ dexp(10),
        sigma_a ~ dexp(10),
        sigma ~ dexp(10)
...
ggsave("../figures/plot_3.png", width = 1500, height = 2000, units = "px")
```
